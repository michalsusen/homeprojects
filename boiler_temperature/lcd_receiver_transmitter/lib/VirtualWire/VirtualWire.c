...
void vw_set_tx_pin(uint16_t pin)
{
    vw_tx_pin = pin;
}

// Set the pin number for input receiver data
void vw_set_rx_pin(uint16_t pin)
{
    vw_rx_pin = pin;
}

// Set the output pin number for transmitter PTT enable
void vw_set_ptt_pin(uint16_t pin)
{
    vw_ptt_pin = pin;
}
...
*/
  ...
  GPIO_InitStructureInput.GPIO_Pin = vw_rx_pin;
  GPIO_InitStructureInput.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructureInput.GPIO_OType = GPIO_OType_OD;
  GPIO_InitStructureInput.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructureInput.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(GPIOC, &GPIO_InitStructureInput);
	
	GPIO_InitStructureOutput.GPIO_Pin = vw_tx_pin | vw_ptt_pin;
  GPIO_InitStructureOutput.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_InitStructureOutput.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructureOutput.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructureOutput.GPIO_PuPd = GPIO_PuPd_NOPULL;
  GPIO_Init(GPIOC, &GPIO_InitStructureOutput);


	GPIO_WriteBit(GPIOC,vw_ptt_pin,(BitAction)vw_ptt_inverted);
...
//    digitalWrite(vw_ptt_pin, true ^ vw_ptt_inverted);
	GPIO_WriteBit(GPIOC,vw_ptt_pin,(BitAction)(true ^ vw_ptt_inverted));
...
//    digitalWrite(vw_tx_pin, false);
		GPIO_WriteBit(GPIOC,vw_ptt_pin,(BitAction)(false ^ vw_ptt_inverted));
		GPIO_WriteBit(GPIOC,vw_tx_pin,(BitAction)(false));
...
SIGNAL(TIMER1_COMPA_vect)
{
        ...
        if (vw_tx_index >= vw_tx_len)
	{
	    ...
	    digitalWrite(vw_tx_pin, vw_tx_buf[vw_tx_index] & (1 << vw_tx_bit++));
			GPIO_WriteBit(GPIOC,vw_tx_pin,(BitAction)(vw_tx_buf[vw_tx_index] & (1 << vw_tx_bit++)));
...
void TIM3_IRQHandler(void)
{
  if (TIM_GetITStatus(TIM3, TIM_IT_CC3) != RESET)
  {
    ...
    TIM_SetCompare3(TIM3, capture + CCR3_Val);
		...
		//vw_rx_sample = digitalRead(vw_rx_pin);
		vw_rx_sample = GPIO_ReadInputDataBit(GPIOC,vw_rx_pin);
